...
class SocialSnap_Social_Follow {
	...
	private function get_follow_count( $network = '' ) {
		...
		if ( isset( $this->configured_networks[ $network ]['profile'] ) && isset( $this->configured_networks[ $network ]['profile']['username'] ) ) {
			$cache = get_transient( 'socialsnap_follow_count_' . $network . '_' . $this->configured_networks[ $network ]['profile']['username'] );
		} 
		...
		// Manual follower count
			...
			if ( $count != $cache && ! is_admin() ) {
				...
				// Save new count
				set_transient( 'socialsnap_follow_count_' . $network . '_' . $network_settings['profile']['username'], $count, 24*60*60 );
			}
		}
	...
	public function get_follow_count_ajax() {
		...
		if ( is_numeric( $result ) && $username ) {
			...
			set_transient( 'socialsnap_follow_count_' . $network . '_' . $username, (int) $result, 24*60*60 );
