...
function socialsnap_perform_remote_request( $action, $body = array(), $headers = array(), $return_format = 'json', $remote_url = SOCIALSNAP_API ) {
	...
	$response      = wp_remote_post( $remote_url, $post );
...
if ( ! function_exists( 'socialsnap_print_notice' ) ) {
	function socialsnap_print_notice( $args ) {
		...
		// Get dismissed info
		if ( get_transient( 'socialsnap_notice_' . $args['message_id'] ) ) {
			return;
		}
...
if ( ! function_exists( 'socialsnap_dismiss_notice' ) ) {
	function socialsnap_dismiss_notice() {
		...
		$message				= (array) get_transient( 'socialsnap_notice_' . $message_id );
		$message['time']		= time();
		$message['dismissed'] 	= true;

		set_transient( 'socialsnap_notice_' . $message_id, $message, $expires );
		die;
	}
}
...
function socialsnap_get_attachment_id_by_url( $url ) {
	...
	$post_id = get_transient( $cache_key, 'socialsnap_attachment_id_by_url' );
	...
	if ( false === $post_id ) {
	    ...
	    set_transient( $cache_key, intval( $post_id ), 'socialsnap_attachment_id_by_url', 12 * HOUR_IN_SECONDS );
	}
